...
MoE的核心思想是**有条件计算（Conditional Computation）**：与其让一个通才模型处理所有问题，不如拥有一组专家，并为每个问题选择最合适的少数几位专家来解决。
这意味着模型拥有海量的总参数（知识存储量大），但在处理任何一个具体的输入时，只激活其中一小部分参数（计算成本低），因此被称为稀疏（Sparse）模型以区别于传统的密集（Dense）模型。
