...
.. glossary::
    ...
    Linear Regression
        线性回归。用于解决回归问题，即预测一个连续的数值输出。线性回归的目标是找到一条直线（或超平面），使其尽可能地拟合训练数据。

    Logistic Regression
        逻辑回归。用于解决分类问题。它通过一个 Sigmoid (Logistic) 函数将线性模型的输出映射到 (0, 1) 区间，表示属于某个类别的概率。
    ...
    Transformer
        一种著名的深度学习模型，最初是作为机器翻译的序列到序列模型提出。基于Transformer的预训练模型在各种自然语言处理任务上实现了最优性能，因此已经成为NLP中的主流架构。
